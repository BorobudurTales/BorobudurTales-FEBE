    <nav x-data="{ open: false }" class="w-full fixed top-0 z-50 bg-gradient-to-r from-[#D78C00] to-[#F1BB6A] px-4 py-2">
        <div class="flex items-center justify-between mx-auto ">
            ...
            <ul class="hidden md:flex gap-x-10 items-center text-white font-medium font-['Inter']">
                ...
                <li><x-users.nav-link href="{{ route('explore') ?? '#' }}" :active="request()->is('explore')">Eksplor</x-users.nav-link>
                </li>
                <li><x-users.nav-link href="{{ route('library') }}" :active="request()->is('library')">Pustaka</x-users.nav-link></li>
                <li><x-users.nav-link href="{{ route('upload') ?? '#' }}" :active="request()->is('upload')">Unggah
                        Gambar</x-users.nav-link>
                </li>
            </ul>
                ...
                @auth
                        ...
                        <div class="relative ml-3" x-data="{ open: false }">
                            <div>
                                <button type="button" @click="open = !open"
                                    ...
                                    <img class="rounded-full size-8" src="{{ asset('img/17.svg') }}" alt="">
                                </button>
                            </div>
                            ...
                            <div x-show="open" @click.outside="open = false"
                                ...
                                <a href="{{ route('profile.edit') }}" class="block px-4 py-2 text-sm text-gray-700"
                                    role="menuitem" tabindex="-1" id="user-menu-item-1">Profil</a>
                    ...
                    @elseif(auth()->check() && auth()->user()->hasRole('admin'))
                        <a href="{{ route('dashboard.admin') }}"
                            class="bg-white text-[#D78C00] px-3 py-1 border border-white rounded text-sm hover:bg-transparent hover:text-white transition-all">Dashboard</a>
                    @endif
                @else
                    <a href="{{ route('login') }}"
        ...
        <div x-show="open" x-transition
            ...
            <div class="pt-2 mt-2 border-t border-white">
                ...
                @else
                    ...
                    <a href="{{ route('register') }}"
                        class="block bg-white text-[#D78C00] rounded px-3 py-1 w-fit">Daftar</a>
                @endauth
            </div>
        </div>
    </nav>
